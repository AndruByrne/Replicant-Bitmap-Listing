...
//since we do not have any onClicks, it is fine to not use fragments as per instructions
	...
	private static final int rows = BitmapListing.TEN_ROWS;
...
//		flinging = false; //never, the asyc relies on being the only one to set this
	...
	public boolean onCreateOptionsMenu( Menu menu ) {
		...
		show100.setOnMenuItemClickListener( new MenuItem.OnMenuItemClickListener( ){
				...
				Intent intent = new Intent(DisplayTen.this, DisplayHundred.class);
		...
		show1000.setOnMenuItemClickListener( new MenuItem.OnMenuItemClickListener( ){
				public boolean onMenuItemClick( MenuItem item ) {
					Intent intent = new Intent(DisplayTen.this, DisplayThousand.class);
					DisplayTen.this.startActivity(intent);
					return false;
				}
			} );
		MenuItem show10000 = menu.add( getString( R.string.show_100000 ) );
		show10000.setShowAsAction( MenuItem.SHOW_AS_ACTION_NEVER );
		show10000.setOnMenuItemClickListener( new MenuItem.OnMenuItemClickListener( ){
				public boolean onMenuItemClick( MenuItem item ) {
					Intent intent = new Intent(DisplayTen.this, DisplayHundredThousand.class);
					DisplayTen.this.startActivity(intent);
					return false;
				}
			} );
